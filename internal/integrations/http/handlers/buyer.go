...
//	@Success		200	{object}	entities.Buyer
...
func (handler *BuyerHandler) Get() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if buyerResponse, err := handler.buyerService.Get(&ctx, id); err != nil {
			...
			web_utils.Response(c, http.StatusOK, buyerResponse)
			return
		}
...
//	@Accept			json
//	@Produce		json
//	@Success		200	{object}	web_utils.response{data=[]entities.Buyer}
//	@Failure		500	{object}	web_utils.errorResponse
...
func (handler *BuyerHandler) GetAll() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if buyers, err := handler.buyerService.GetAll(&ctx); err != nil {
			...
			if len(*buyers) == 0 {
				web_utils.Success(c, http.StatusNoContent, buyers)
				return
			}
			web_utils.Success(c, http.StatusOK, buyers)
			return
		}
	}
}
...
//	@Accept			json
//	@Produce		json
//	@Param			SellerHandler	body		dtos.CreateBuyerRequestDTO	true	"Buyer to Create"
//	@Success		201		{object}	entities.Buyer
...
func (handler *BuyerHandler) Create() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if createdBuyer, err := handler.buyerService.Create(&ctx, createBuyerRequest); err != nil {
			...
			web_utils.Response(c, http.StatusCreated, createdBuyer)
			return
		}
	}
}
...
func (handler *BuyerHandler) Update() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if updatedBuyer, err := handler.buyerService.Update(&ctx, id, updateBuyerRequest); err != nil {
			...
			web_utils.Response(c, http.StatusOK, updatedBuyer)
			return
		}
	}
}
